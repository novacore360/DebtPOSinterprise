...
import { Plus, Trash2, Barcode, ShoppingCart, CheckCircle, X, Search } from 'lucide-react';
...
const lbl  = { color:'rgba(255,255,255,0.55)', fontSize:12, marginBottom:5, display:'block', fontWeight:600, letterSpacing:.3 };
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16, maxWidth:'100%' }}>
      ...
      {toast && (
        <div style={{
          ...
          background:'rgba(231,74,59,0.95)', color:'#fff', padding:'10px 18px', borderRadius:8,
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))', gap:14, alignItems:'start' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          {/* Customer */}
          <div style={card}>
            <p style={{ color:'#fff', fontWeight:700, margin:'0 0 14px', fontSize:14 }}>Customer</p>
            ...
            <div style={{ marginBottom:10, position:'relative' }}>
              ...
              <div style={{ position:'relative' }}>
                ...
                {selectedCustomer && (
                  <button onClick={clearCustomer} style={{ position:'absolute', right:8, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'rgba(255,255,255,0.4)', cursor:'pointer', padding:4 }}>
                    <X size={13} />
                  </button>
                )}
              </div>
              {custSuggestions.length > 0 && (
                <div style={{ position:'absolute', zIndex:200, background:'#1e2035', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, width:'100%', maxHeight:180, overflowY:'auto', boxShadow:'0 8px 24px rgba(0,0,0,0.5)' }}>
                  {custSuggestions.map(c => (
                    <div key={c.id} onClick={() => selectCustomer(c)}
                      style={{ padding:'9px 12px', cursor:'pointer', borderBottom:'1px solid rgba(255,255,255,0.05)', color:'#fff', fontSize:13 }}
                      onMouseEnter={e => e.currentTarget.style.background='rgba(78,115,223,0.2)'}
                      onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                      <div style={{ fontWeight:600 }}>{c.name}</div>
                      {c.phone && <div style={{ color:'rgba(255,255,255,0.4)', fontSize:11 }}>{c.phone}</div>}
                    </div>
                  ))}
                </div>
              )}
          ...
          {/* Add Product */}
          <div style={card}>
            <p style={{ color:'#fff', fontWeight:700, margin:'0 0 14px', fontSize:14 }}>Add Product</p>

            <div style={{ marginBottom:10, position:'relative' }}>
              <label style={lbl}>Search by name or barcode</label>
              <div style={{ position:'relative' }}>
                <Search size={13} style={{ position:'absolute', left:11, top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.35)' }} />
              ...
              {productSuggestions.length > 0 && (
                <div style={{ position:'absolute', zIndex:200, background:'#1e2035', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, width:'100%', maxHeight:220, overflowY:'auto', boxShadow:'0 8px 24px rgba(0,0,0,0.5)' }}>
                  {productSuggestions.map(p => {
                    const price = p.retailPrice || p.price || 0;
                    return (
                      <div key={p.id} onClick={() => selectProduct(p)}
                        style={{ padding:'10px 12px', cursor:'pointer', borderBottom:'1px solid rgba(255,255,255,0.05)', color:'#fff', fontSize:13 }}
                        onMouseEnter={e => e.currentTarget.style.background='rgba(78,115,223,0.2)'}
                        onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                        <div style={{ display:'flex', justifyContent:'space-between', gap:8, flexWrap:'wrap' }}>
                          <span style={{ fontWeight:600 }}>{p.name}
                            <span style={{ color:'rgba(255,255,255,0.35)', fontSize:11, marginLeft:6 }}>{p.productCode}</span>
                          </span>
                          <span style={{ color:'#1cc88a', fontWeight:700 }}>₱{price.toFixed(2)}</span>
                        </div>
                        <div style={{ color:(p.stock||0)===0?'#e74a3b':'rgba(255,255,255,0.4)', fontSize:11, marginTop:2 }}>
                          Stock: {p.stock||0}
                        </div>
                      </div>
                    );
            ...
            {selectedProduct && (
              <div style={{ background:'rgba(78,115,223,0.1)', border:'1px solid rgba(78,115,223,0.3)', borderRadius:9, padding:'9px 11px', marginBottom:10, fontSize:13 }}>
                <div style={{ color:'#fff', fontWeight:600 }}>{selectedProduct.name}</div>
                <div style={{ display:'flex', justifyContent:'space-between', marginTop:3, flexWrap:'wrap', gap:6 }}>
                  <span style={{ color:'rgba(255,255,255,0.5)' }}>₱{(selectedProduct.retailPrice||selectedProduct.price||0).toFixed(2)}</span>
        ...
        <div style={card}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14, flexWrap:'wrap', gap:8 }}>
            <p style={{ color:'#fff', fontWeight:700, margin:0, display:'flex', alignItems:'center', gap:6, fontSize:14 }}>
          ...
          {cart.length === 0 ? (
            ...
            <>
                  ...
                  <thead>
                    <tr>
                      {['Product','Price','Qty','Sub',''].map((h, i) => (
                        <th key={i} style={{ color:'rgba(255,255,255,0.4)', fontWeight:600, textAlign: i===1||i===3?'right':'left', padding:'5px 8px', borderBottom:'1px solid rgba(255,255,255,0.07)', fontSize:11 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
              ...
              {/* Total */}
              <div style={{ borderTop:'1px solid rgba(255,255,255,0.07)', paddingTop:12, display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              ...
              <button onClick={finalizePurchase} disabled={finalizing} style={{
                ...
                border:'none', borderRadius:11, color:'#fff', fontWeight:700,
      ...
      {receipt && (
        <div style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(0,0,0,0.88)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div style={{ background:'#1a1a2e', borderRadius:16, padding:22, maxWidth:420, width:'100%', border:'1px solid rgba(255,255,255,0.1)', maxHeight:'90vh', overflowY:'auto' }}>
            <div style={{ textAlign:'center', marginBottom:18 }}>
              <CheckCircle size={44} color="#1cc88a" style={{ marginBottom:8 }} />
              <h4 style={{ color:'#fff', margin:0 }}>Purchase Complete!</h4>
              <p style={{ color:'rgba(255,255,255,0.45)', margin:'4px 0 0', fontSize:12 }}>Status: <b style={{ color:'#f6c23e' }}>PENDING</b></p>
            </div>
            <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:11, padding:14, marginBottom:14 }}>
              <div style={{ color:'rgba(255,255,255,0.55)', fontSize:13, marginBottom:8 }}>
                Customer: <b style={{ color:'#fff' }}>{receipt.customer.name}</b>
              </div>
              {receipt.items.map((item, i) => (
                <div key={i} style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'rgba(255,255,255,0.7)', marginBottom:4, flexWrap:'wrap', gap:4 }}>
                  <span>{item.name} × {item.quantity}</span>
                  <span>₱{item.subtotal.toFixed(2)}</span>
                </div>
              ))}
              <div style={{ borderTop:'1px dashed rgba(255,255,255,0.15)', marginTop:10, paddingTop:10, display:'flex', justifyContent:'space-between' }}>
                <b style={{ color:'#fff' }}>TOTAL</b>
                <b style={{ color:'#1cc88a', fontSize:16 }}>₱{receipt.totalAmount.toFixed(2)}</b>
              </div>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              <button onClick={printReceipt} style={{ flex:1, padding:11, background:'rgba(78,115,223,0.2)', border:'1px solid rgba(78,115,223,0.3)', borderRadius:9, color:'#4e73df', fontWeight:600, cursor:'pointer', fontSize:14 }}>
                🖨 Print
              </button>
              <button onClick={() => setReceipt(null)} style={{ flex:1, padding:11, background:'rgba(255,255,255,0.08)', border:'none', borderRadius:9, color:'#fff', fontWeight:600, cursor:'pointer', fontSize:14 }}>
                Close
              </button>
            </div>
          </div>
